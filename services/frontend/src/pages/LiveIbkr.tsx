/**
 * Live IBKR tab — shows the state of the connected IB Gateway / TWS
 * session (auto-detected — paper or live, no config switch). All data
 * here is read from /live-ibkr/* endpoints which mirror what the
 * background sync wrote to ibkr_live.sqlite.
 *
 * The mode badge is the single most important UI element on this page:
 *   - gray "DISCONNECTED" if no recent sync
 *   - emerald "PAPER" if mode === 'paper'
 *   - red "LIVE" if mode === 'live' (deliberately loud — losing focus on
 *     mode is the #1 way to fire orders at the wrong account)
 */
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { useState } from 'react';

const API_BASE = '';  // proxied through vite.config; same origin

type Status = {
  connected: boolean;
  mode: 'paper' | 'live' | null;
  surface: 'gateway' | 'tws' | null;
...
type EquityPoint = { snapshot_at: string; nav: number; cash: number; long_mv: number };
...
export function LiveIbkrPage() {
  ...
  const status = useQuery<Status>({
    ...
    queryFn: () => fetch(API_BASE + '/live-ibkr/status').then(r => r.json()),
    refetchInterval: 5000,
  });
  const positions = useQuery<Position[]>({
    queryKey: ['ibkr', 'positions'],
    queryFn: () => fetch(API_BASE + '/live-ibkr/positions').then(r => r.json()),
    refetchInterval: 10000,
  });
  const orders = useQuery<OrderRow[]>({
    queryKey: ['ibkr', 'orders'],
    queryFn: () => fetch(API_BASE + '/live-ibkr/orders/today').then(r => r.json()),
    refetchInterval: 10000,
  });
  const equity = useQuery<EquityPoint[]>({
    queryKey: ['ibkr', 'equity'],
    queryFn: () => fetch(API_BASE + '/live-ibkr/equity').then(r => r.json()),
    refetchInterval: 30000,
  });
  const recon = useQuery<ReconResp>({
    queryKey: ['ibkr', 'reconciliation'],
    queryFn: () => fetch(API_BASE + '/live-ibkr/reconciliation').then(r => r.json()),
    refetchInterval: 30000,
  });
  const pending = useQuery<PendingSignal[]>({
    queryKey: ['ibkr', 'pending'],
    queryFn: () => fetch(API_BASE + '/live-ibkr/signals/pending').then(r => r.json()),
    refetchInterval: 5000,
  });

  const approveMut = useMutation({
    mutationFn: async (payload: { signal_ids: number[]; confirm_live_account_id: string | null }) => {
      const r = await fetch(API_BASE + '/live-ibkr/signals/approve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          signal_ids: payload.signal_ids,
          approved_by: 'dashboard-user',
          confirm_live_account_id: payload.confirm_live_account_id,
        }),
      });
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['ibkr'] });
    },
  });
  const rejectMut = useMutation({
    mutationFn: async (ids: number[]) => {
      const r = await fetch(API_BASE + '/live-ibkr/signals/reject', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleApprove = () => {
    ...
    if (isLive && confirmAccount !== s?.account_id) {
      alert(`LIVE mode requires you to type the connected account id (${s?.account_id}) to confirm.`);
      return;
    }
    approveMut.mutate({
      signal_ids: Array.from(selectedSignals),
      confirm_live_account_id: isLive ? s!.account_id : null,
    });
  };
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-gray-100">Live IBKR</h1>
            <p className="text-xs text-gray-400">
              Auto-detected from running Gateway / TWS. Switch modes by switching what you log into in Gateway.
            </p>
          </div>
          <ModeBadge status={s} />
        </div>
        {s && (
          <div className="mt-3 grid grid-cols-2 gap-3 text-xs sm:grid-cols-5">
            ...
            <Field label="Account" value={s.account_id ?? '—'} mono />
            <Field label="Surface" value={s.surface ?? '—'} />
            <Field label="Port" value={s.port ? String(s.port) : '—'} mono />
        ...
        {isLive && (
          <div className="mt-3 rounded border border-rose-500 bg-rose-950 p-3 text-xs text-rose-200">
            ...
            <code className="font-mono">{s.account_id}</code>. To approve any signal you must type the account id below.
          </div>
        )}
      </section>
            ...
            <tbody className="divide-y divide-gray-800">
              {positions.data.map(p => (
                <tr key={p.symbol}>
                  <td className="py-1 font-mono text-sm text-gray-200">{p.symbol}</td>
                  <td className="py-1 text-right font-mono text-xs">{fmt(p.qty, 0)}</td>
                  <td className="py-1 text-right font-mono text-xs text-gray-400">{fmtUsd(p.avg_cost)}</td>
        ...
        {orders.data && orders.data.length > 0 ? (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-800">
              {orders.data.map(o => (
                <tr key={o.order_id}>
                  <td className="py-1 font-mono text-xs text-gray-400">{new Date(o.placed_at).toLocaleTimeString()}</td>
                  ...
                  <td className="py-1 text-right font-mono text-xs">{fmtUsd(o.avg_fill_price)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ) : (
          <p className="text-xs text-gray-500">No orders placed today.</p>
        )}
      </section>
      ...
      <section className="rounded-lg border border-gray-800 bg-gray-900/60 p-3">
        <h2 className="mb-2 text-[11px] uppercase tracking-wider text-gray-500">
          Reconciliation — Kubera paper vs IBKR actual
        </h2>
        {recon.data && (
